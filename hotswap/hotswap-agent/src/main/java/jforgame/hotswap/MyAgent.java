...
public class MyAgent {
    ...
    public static void agentmain(String args, Instrumentation inst) {
        Class<?> agent = null;
        try {
            agent = Class.forName("jforgame.hotswap.JavaDoctor");
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        }
        DataInputStream dis = null;
        try {
            Field field = agent.getDeclaredField("fixData");
            ...
            sb.append( "]完成");
            Field field2 = agent.getDeclaredField("log");
            field2.setAccessible(true);
            field2.set(null, sb.toString());
        } catch (Exception e) {
            try {
                Field field = agent.getDeclaredField("exception");
                field.setAccessible(true);
                field.set(null, e);
            } catch (Exception e1) {
                e1.printStackTrace();
